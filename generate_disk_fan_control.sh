...
echo "📝 请编辑 generate_disk_fan_control.sh 中的 PWM_NODES 数组以适配您的系统 PWM 节点（如 pwm3、pwm4 等），然后重新运行生成脚本。"
echo "ℹ️ 示例：将 PWM3=\"\${HWMON_PATH}/pwm3\" 改为实际节点（如 pwm1、pwm2），可添加更多节点（如 PWM5=\"\${HWMON_PATH}/pwm5\"）。"
echo "ℹ️ 运行 'ls /sys/class/hwmon/*/pwm*' 查看可用 PWM 节点。"
echo "ℹ️ 运行 'smartctl -A /dev/nvme0n1' 或 'smartctl -A /dev/sdX' 查看实际温度字段，更新 NVME_TEMP_FIELDS 和 HDD_TEMP_FIELD 在 generate_disk_fan_control.sh 中，然后重新运行生成脚本。"
